...
public class ImageIndicatorView extends RelativeLayout {
     ...
     * @param resList Drawable列表
     */
    public void setDrawable(final List<String > resList) {
        if (resList == null)
            throw new NullPointerException();
        ...
        if (len > 0) {
            for (int index = 0; index < len; index++) {
                ...
                ToolImage.getImageLoader().displayImage(resList.get(index),pageItem,ToolImage.getDefaultFailureOptions());
                addViewItem(pageItem);
            }
        }
    }
    ...
    public void setAutomatic(boolean flag, long delay, long period) {
        if (flag) {
            /**
             * task--这是被调度的任务。
    ...
    Handler handler = new Handler() {
        public void handleMessage(Message msg) {
            if (msg.what == 1) {
                ...
                setCurrentItemViewPage(currentIndex);
                currentIndex++;
                if (currentIndex >= totelCount) {
                    currentIndex = 0;
                }
